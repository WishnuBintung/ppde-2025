...
class DataVisualizer:
    def __init__(self, root):
        ...
        # Variabel kontrol (dibuat duluan supaya siap dipakai di update_plot)
    ...
    def setup_ui(self):
        ...
        control_container = tk.Frame(main_frame, bg="lightblue", width=250)
        control_container.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10), anchor="nw")

        canvas = tk.Canvas(control_container, bg="lightblue", width=250, highlightthickness=0)
        scrollbar = tk.Scrollbar(control_container, orient="vertical", command=canvas.yview)
        self.control_frame = tk.Frame(canvas, bg="lightblue", width=250)

        # Update scrollregion setiap kali frame berubah ukuran
    ...
    def setup_controls(self):
        # --- Fungsi Matematis ---
        tk.Label(self.control_frame, text="Frekuensi:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        tk.Scale(self.control_frame, from_=0.1, to=5.0, resolution=0.1,
                orient=tk.HORIZONTAL, variable=self.frequency,
                command=self.on_parameter_change, length=200, bg="lightblue").pack()
        self.freq_value_label = tk.Label(self.control_frame, text=f"Nilai: {self.frequency.get()}",
                                        font=("Arial", 10), bg="lightblue")
        self.freq_value_label.pack()

        tk.Label(self.control_frame, text="Amplitudo:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        tk.Scale(self.control_frame, from_=0.1, to=3.0, resolution=0.1,
                orient=tk.HORIZONTAL, variable=self.amplitude,
                command=self.on_parameter_change, length=200, bg="lightblue").pack()
        self.amp_value_label = tk.Label(self.control_frame, text=f"Nilai: {self.amplitude.get()}",
                                        font=("Arial", 10), bg="lightblue")
        self.amp_value_label.pack()

        tk.Label(self.control_frame, text="Phase:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        tk.Scale(self.control_frame, from_=0.0, to=6.28, resolution=0.1,
                orient=tk.HORIZONTAL, variable=self.phase,
                command=self.on_parameter_change, length=200, bg="lightblue").pack()
        self.phase_value_label = tk.Label(self.control_frame, text=f"Nilai: {self.phase.get()}",
                                        font=("Arial", 10), bg="lightblue")
        self.phase_value_label.pack()

        tk.Label(self.control_frame, text="Jenis Fungsi:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        function_combo = ttk.Combobox(self.control_frame, textvariable=self.function_type,
            values=["sin", "cos", "tan", "exp", "log"], state="readonly", width=18)
        function_combo.pack()
        function_combo.bind("<<ComboboxSelected>>", self.on_function_change)

        tk.Label(self.control_frame, text="Warna Garis:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        color_combo = ttk.Combobox(self.control_frame, textvariable=self.line_color,
            values=["blue", "red", "green", "orange", "purple", "brown", "pink"],
            state="readonly", width=18)
        color_combo.pack()
        color_combo.bind("<<ComboboxSelected>>", self.on_style_change)

        tk.Label(self.control_frame, text="Style Garis:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        style_combo = ttk.Combobox(self.control_frame, textvariable=self.line_style,
            values=["-", "--", "-.", ":", "o-", "s-", "^-"],
            state="readonly", width=18)
        style_combo.pack()
        style_combo.bind("<<ComboboxSelected>>", self.on_style_change)

        tk.Label(self.control_frame, text="Ketebalan Garis:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        tk.Scale(self.control_frame, from_=0.5, to=5.0, resolution=0.5,
            orient=tk.HORIZONTAL, variable=self.line_width,
            command=self.on_style_change, length=200, bg="lightblue").pack()

        # --- Data Controls ---
        separator = tk.Frame(self.control_frame, height=2, bg="darkblue")
        separator.pack(fill=tk.X, pady=15)

        tk.Button(self.control_frame, text="Load Data CSV", font=("Arial", 11, "bold"),
            command=self.load_data, bg="orange", fg="white", width=20).pack(pady=5)
        tk.Button(self.control_frame, text="Generate Sample Data", font=("Arial", 11),
            command=self.generate_sample_data, bg="green", fg="white", width=20).pack(pady=5)

        self.data_info_label = tk.Label(self.control_frame, text="Belum ada data dimuat",
            font=("Arial", 9), bg="lightblue", wraplength=200)
        self.data_info_label.pack(pady=5)

        tk.Label(self.control_frame, text="Jenis Plot:", font=("Arial", 12, "bold"), bg="lightblue").pack(pady=(10, 5))
        plot_type_combo = ttk.Combobox(self.control_frame, textvariable=self.plot_type,
            values=["line", "scatter", "bar", "histogram", "box"],
            state="readonly", width=18)
        plot_type_combo.pack()
        plot_type_combo.bind("<<ComboboxSelected>>", self.on_plot_type_change)

        self.subplot_check = tk.Checkbutton(self.control_frame, text="Gunakan Multiple Subplot",
            variable=self.use_subplot, command=self.toggle_subplot,
            font=("Arial", 10), bg="lightblue")
        self.subplot_check.pack(pady=5)

        separator2 = tk.Frame(self.control_frame, height=2, bg="darkblue")
        separator2.pack(fill=tk.X, pady=15)

        tk.Label(self.control_frame, text="Real-time Simulation:", font=("Arial", 12, "bold"),
            bg="lightblue").pack(pady=(5, 5))

        self.realtime_btn = tk.Button(
            self.control_frame, text="Start Real-time", font=("Arial", 11),
            command=self.toggle_realtime, bg="red", fg="white", width=20
        )
        self.realtime_btn.pack(pady=5)

        tk.Label(self.control_frame, text="Update Speed (ms):", font=("Arial", 10),
            bg="lightblue").pack(pady=(5, 2))

        tk.Scale(self.control_frame, from_=100, to=2000, resolution=100,
            orient=tk.HORIZONTAL, variable=self.update_speed,
            length=180, bg="lightblue").pack()

    # === Event Handlers ===
    def on_style_change(self, event=None): self.update_plot()
    def on_function_change(self, event=None): self.update_plot()
    ...
    # === Plotting Fungsi Matematis ===
    def update_plot(self):
        self.ax.clear()
        x = np.linspace(0, 10, 100)
        f, a, p = self.frequency.get(), self.amplitude.get(), self.phase.get()
        func, color, style, width = self.function_type.get(), self.line_color.get(), self.line_style.get(), self.line_width.get()

        try:
            if func == "sin":
                y = a * np.sin(f * x + p); title = f"y = {a} × sin({f}x + {round(p, 2)})"
            elif func == "cos":
                y = a * np.cos(f * x + p); title = f"y = {a} × cos({f}x + {round(p, 2)})"
            elif func == "tan":
                y = np.clip(a * np.tan(f * x + p), -10, 10); title = f"y = {a} × tan({f}x + {round(p, 2)})"
            elif func == "exp":
                y = np.clip(a * np.exp(f * (x - 5) + p), 0, 100); title = f"y = {a} × exp({f}(x-5) + {round(p, 2)})"
            elif func == "log":
                y = a * np.log(f * x + 1) + p; title = f"y = {a} × log({f}x + 1) + {round(p, 2)}"
        except:
            y = a * np.sin(f * x + p); title = f"y = {a} × sin({f}x + {round(p, 2)})"

        self.ax.plot(x, y, linestyle=style, color=color, linewidth=width)
        self.ax.set_title(title); self.ax.set_xlabel("X"); self.ax.set_ylabel("Y"); self.ax.grid(True, alpha=0.3)
        self.canvas.draw()
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                ...
                self.data_info_label.config(text=f"Data dimuat: {rows} baris, {cols} kolom")
                self.plot_data()
    ...
    def generate_sample_data(self):
        ...
        sales = np.linspace(100, 200, len(dates)) + 30 * np.sin(2 * np.pi * np.arange(len(dates)) / 365.25) + np.random.normal(0, 15, len(dates))
        temp = 25 + 10 * np.sin(2 * np.pi * np.arange(len(dates)) / 365.25) + np.random.normal(0, 5, len(dates))
        self.current_data = pd.DataFrame({"Date": dates, "Sales": sales, "Temp": temp})
        r, c = self.current_data.shape
        self.data_info_label.config(text=f"Sample data: {r} baris, {c} kolom")
        self.plot_data()

    def plot_data(self): self.plot_data_advanced()
    def plot_data_advanced(self):
        ...
        self.ax.clear()
        cols = self.current_data.select_dtypes(include=[np.number]).columns
        kind = self.plot_type.get()

        # 🔥 Ambil warna sesuai jumlah kolom numeric
        colors = self.get_colors(len(cols))

        if kind == "line" and len(cols) >= 1:
        ...
        elif kind == "scatter" and len(cols) >= 2:
            self.ax.scatter(
                self.current_data[cols[0]],
                self.current_data[cols[1]],
                color=colors[0]
            )

        elif kind == "bar" and len(cols) >= 1:
            self.ax.bar(
                range(len(self.current_data.head(20))),
                self.current_data[cols[0]].head(20),
                color=colors[0]
            )

        elif kind == "histogram" and len(cols) >= 1:
            self.ax.hist(
                self.current_data[cols[0]].dropna(),
                bins=20,
                color=colors[0]
            )

        elif kind == "box" and len(cols) >= 1:
            self.ax.boxplot(
                [self.current_data[col].dropna() for col in cols],
                patch_artist=True,
                boxprops=dict(facecolor=colors[0], color=colors[0])
            )

        self.ax.set_title(f"{kind.title()} Plot")
        self.ax.grid(True, alpha=0.3)
        self.canvas.draw()
        self.update_performance_info(start_time)
        self.update_status("Plot ready")

    def on_plot_type_change(self, e=None):
        if self.current_data is not None: self.plot_data_advanced()

    def toggle_subplot(self):
        if self.current_data is not None: self.plot_data_advanced()
        else: messagebox.showwarning("Peringatan", "Muat data terlebih dahulu!")

    # === Real-time ===
    def toggle_realtime(self):
        if not self.realtime_active.get():
            self.realtime_active.set(True); self.realtime_btn.config(text="Stop Real-time", bg="green")
            self.start_realtime_thread()
        else:
            self.realtime_active.set(False); self.realtime_btn.config(text="Start Real-time", bg="red")

    def start_realtime_thread(self):
        def loop():
            while self.realtime_active.get():
                t = len(self.realtime_data)
                v1 = 50 + 20 * np.sin(t * 0.1) + np.random.normal(0, 5)
                v2 = 30 + 15 * np.cos(t * 0.15) + np.random.normal(0, 3)
                v3 = 40 + 10 * np.sin(t * 0.08) + np.random.normal(0, 4)
                self.realtime_data.append({"t": t, "s1": v1, "s2": v2, "s3": v3})
                if len(self.realtime_data) > self.max_points: self.realtime_data.pop(0)
                self.root.after(0, self.update_realtime_plot)
                time.sleep(self.update_speed.get() / 1000.0)
        threading.Thread(target=loop, daemon=True).start()

    def update_realtime_plot(self):
        if not self.realtime_data: return
        t = [d["t"] for d in self.realtime_data]
        s1, s2, s3 = [d["s1"] for d in self.realtime_data], [d["s2"] for d in self.realtime_data], [d["s3"] for d in self.realtime_data]
        self.ax.clear()
        self.ax.plot(t, s1, "b-", label="Sensor 1"); self.ax.plot(t, s2, "r-", label="Sensor 2"); self.ax.plot(t, s3, "g-", label="Sensor 3")
        self.ax.legend(); self.ax.set_title("Real-time Data"); self.ax.grid(True, alpha=0.3)
        self.canvas.draw()
    ...
    def setup_theme_controls(self):
        ...
        style_combo.bind("<<ComboboxSelected>>", self.on_theme_style_change)
    ...
    def on_style_change(self, event=None):
        """Update hanya line style (garis, warna, ketebalan)"""
        self.update_plot()

    def on_color_scheme_change(self, event=None):
        """Update color scheme (akan diterapkan saat plot ulang)"""
        if hasattr(self, 'current_data') and self.current_data is not None:
            self.plot_data_advanced()
        else:
            self.update_plot()

    def on_theme_style_change(self, event=None):
        """Update style global (tema matplotlib)"""
        style = self.plot_style.get()
        try:
            plt.style.use(style)
            # Replot agar style baru langsung terpakai
            if hasattr(self, 'current_data') and self.current_data is not None:
                self.plot_data_advanced()
            else:
                self.update_plot()
